...
module Micro
  class Case
    ...
    INVALID_INVOCATION_OF_THE_THEN_METHOD =
      Error::InvalidInvocationOfTheThenMethod.new("#{name}.")
    ...
    def self.then(use_case = nil, &block)
      ...
      if block
        ...
        raise NotImplementedError unless can_yield_self
    ...
    def self.inherited(subclass)
      subclass.__attributes_set_after_inherit__(__attributes_data__)

      subclass.extend ::Micro::Attributes.const_get('Macros::ForSubclasses')

      return unless send(:__flow_use_cases) && !subclass.name.to_s.end_with?(FLOW_STEP)
    ...
    private_class_method def self.__flow_set(args)
      ...
      def self.use_cases
        __flow_get__.use_cases
      end

      class_eval('def use_cases; self.class.use_cases; end', __FILE__, __LINE__)
    ...
    def self.__call__!
      ...
      class_eval("class #{FLOW_STEP} < #{name}; private def __call; __call_use_case; end; end; #{FLOW_STEP}", __FILE__,
                 __LINE__ - 1)
    end
    ...
    private_class_method def self.__flow_use_cases_get
      Array(__flow_use_cases)
        .map { |use_case| use_case == self ? __call__! : use_case }
    end
    ...
    def self.inspect
      ...
      return format('#<%s: ...>', self) if ids.include?(object_id)

      begin
        ids << object_id

        if __flow_use_cases
          format('<%s (%s) use_cases=%s>', self, __flow_builder__, @__flow_use_cases)
        else
          format('<%s (%s) attributes=%s>', self, superclass, attributes)
        end
      ensure
        ids.pop
      end
    ...
    def __call_use_case
      ...
      raise Error::UnexpectedResult, "#{self.class.name}#call!"
    end
    ...
    MapFailureType = lambda do |value, type|
